...
- **Combines two parts of one scan.** Enable **More → Combine Two Parts** for that file, draw the first region and then the second, and adjust each region’s four corners independently. Choose **Vertical** or **Horizontal**; the first region stays above or to the left of the second. **Redraw Parts** starts a new selection order. The result has a white background, margins and a gap, and **Save JPG…** writes a separate image. This mode is never enabled or changed by automatic detection.
- **Shows a live result preview** in the editor’s upper-right corner, including both parts rectified and combined. The grid and filmstrip show the same result.
