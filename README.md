...
- **Auto-straightens only when asked.** Opening a folder does not run detection or propose new crops. Click **Auto-Straighten All**, **Auto-Straighten** in the page editor, or **Auto-Straighten Selected** in the grid to find each document and propose framing. Uses the Vision framework built into macOS, so nothing is sent anywhere and there is nothing to install. A sheet lying crooked comes back with the angle that puts it upright; one photographed at an angle is squared up by its four corners, which you can drag onto the real ones. Suggestions that would cover almost the whole scan are discarded rather than shaving a sliver off a page that needed no cropping.
- **Rejects framing with Esc.** In the editor, Esc clears the current page’s crop, perspective correction, tilt and rotation, and keeps that page outside the common format until framing is requested again. **All Pages** returns to the grid keeping your edits.
- **Combines up to four parts of one scan.** **Auto-Straighten** looks for separate document regions and selects a two-, three- or four-part composition when it finds them. In the editor, press **1–4** or use **Page → 1 Part / 2 Parts / 3 Parts / 4 Parts** (also in **More → Number of Parts**) to choose the mode yourself. Draw any missing regions and adjust their corners independently. Choose **Vertical** or **Horizontal** for the combined image. Press **C** or choose **Cycle Part Order** to move the first part to the end. Hold **C** and click parts to assign first, second, third and fourth place; the remaining numbers update automatically, keeping their relative order. Releasing C after clicking keeps that order. **Redraw Parts** clears the regions. The result has a white background, outer margins and a 2-pixel gap between parts. **Save JPG…** writes a separate image; **Apply…** in the top toolbar replaces the source file with the combined result.
- **Shows a live result preview** in the editor’s upper-right corner, including all parts rectified and combined. The grid and filmstrip show the same result.
...
- **Straightens a crooked scan by hand** with the tilt slider, for the ones the detector missed. Turning a page nobody has framed frames it as it goes: the crop is the largest upright rectangle the turn leaves inside the scan, so the page comes out square and whole with none of the empty corners a turn brings in. Back to zero and the frame lets go again; a crop you drew yourself is never replaced, only moved and shrunk as far as the turn demands.
...
- **Processes a selection.** ⌘-click individual pages or Shift-click a range in the grid. **Auto-Straighten Selected** and **Apply Selected to Originals…** act on that selection independently. Unselected pages keep their pending edits and sources; a selected page sharing their scan receives a separate file when applied.
...
- **Apply changes to original files** — rewrites the sources with their crops baked in. Confirmed by a dialog, and untouched copies are kept in `.id-fit-originals/` unless you opt out. Images are re-encoded in their original format; PDFs are cropped through their crop box, so they stay vector and lose no pages. When one scan holds two pages of the document — a cover that is both the front and the back — one file cannot hold both framings, so each of those pages is given a copy of its own beside it (`scan-2.png`), and the document follows them there. Composed pages replace their source in the same format and under the same filename, just like ordinary crops. For PDF sources, only the affected page is replaced by the combined image inside the existing PDF. Extra files are created only for duplicated pages sharing a source. Incomplete compositions are skipped until all regions are drawn.
